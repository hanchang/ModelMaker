...
class Auth extends Controller
{
	...
	function register()
	{
		if ($this->tank_auth->is_logged_in()) {									// logged in
			...
			$this->template->load('template', 'auth/register_form', $data);
		}
	}
	...
	function reset_password()
	{
		...
		$this->form_validation->set_rules(
			'new_password', 'New Password', 'trim|required|xss_clean|min_length['.$this->config->item('password_min_length', 'tank_auth').']|max_length['.$this->config->item('password_max_length', 'tank_auth').']');
	...
	function change_email()
	{
		if (!$this->tank_auth->is_logged_in()) {								// not logged in or not activated
			...
			$this->template->load('template', 'auth/change_email_form', $data);
		}
	}
	...
	function unregister()
	{
		if (!$this->tank_auth->is_logged_in()) {								// not logged in or not activated
			...
			$this->template->load('template', 'auth/unregister_form', $data);
		}
	}
	...
	function cp()
	{
		...
		if ($user_id === FALSE) { // not logged in or not activated
			redirect('auth/login');
		} else {
			...
			$data['upcoming_cases'] = $this->case_model->getCurrentCasesByDeadline(3);
			...
			if (empty($profile->pic_face) && empty($profile->pic_upper) && empty($profile->pic_full)) {
				$data['photos'] = FALSE;
			}
			else {
				$data['photos'] = TRUE;
			...
			$this->template->load('template', 'auth/control_panel', $data);
		}
	}
	...
	{
		$this->template->load('template', 'auth/general_message', array('message' => $message));
	}
	...
	function _send_email($type, $email, &$data)
	{
		...
		$config['smtp_user'] = 'pillarofeden';
		$config['smtp_pass'] = 'W3bF@ct10n';
